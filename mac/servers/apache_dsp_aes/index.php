...
function log_trace(string $path, int $arrivalNs, float $serviceMs,
                   float $queueMs, float $responseMs, int $statusCode,
                   array $stageMs, string $route, string $method): void {
    $dir = dirname($path);
    ...
    if ($stat !== false && $stat['size'] === 0) {
        fwrite($fp, "arrival_unix_ns,service_ms,queue_ms,response_ms,status_code,route,method,pid,parse_ms,decrypt_ms,unpack_ms,kernel_ms,fir_ms,pack_ms,encrypt_ms\n");
    }
    fwrite($fp, sprintf("%d,%.6f,%.6f,%.6f,%d,%s,%s,%d,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f\n",
        $arrivalNs, $serviceMs, $queueMs, $responseMs, $statusCode,
        str_replace(',', ';', $route), str_replace(',', ';', $method), getmypid(),
        (float)($stageMs['parse_ms'] ?? 0.0),
        (float)($stageMs['decrypt_ms'] ?? 0.0),
        (float)($stageMs['unpack_ms'] ?? 0.0),
        (float)($stageMs['kernel_ms'] ?? 0.0),
        (float)($stageMs['fir_ms'] ?? 0.0),
        (float)($stageMs['pack_ms'] ?? 0.0),
        (float)($stageMs['encrypt_ms'] ?? 0.0)));
    fflush($fp);
    flock($fp, LOCK_UN);
    fclose($fp);
}
...
if ($csvFile !== '') {
    ...
    register_shutdown_function(
        function() use ($requestStartNs, $arrivalNs, $csvFile, &$actualServiceMsRef, &$stageMs, &$path, &$method) {
            ...
            log_trace($csvFile, $arrivalNs, $actualServiceMsRef, $queueMs,
                      $totalResponseMs, http_response_code(), $stageMs, $path ?? '', $method ?? '');
        }
    );
}

// --- Step 1: Parse request ---
$parseStartNs = hrtime(true);
$body = file_get_contents('php://input');
$req  = json_decode($body, true);
if (!is_array($req) || empty($req['iv']) || empty($req['payload'])) {
    $stageMs['parse_ms'] = elapsed_ms($parseStartNs, hrtime(true));
...
$stageMs['decrypt_ms'] = elapsed_ms($stageStartNs, hrtime(true));
...
$stageMs['unpack_ms'] = elapsed_ms($stageStartNs, hrtime(true));
if ($nSamples === 0) {
    $serviceEndNs = hrtime(true);
...
$stageMs['kernel_ms'] = elapsed_ms($stageStartNs, hrtime(true));
