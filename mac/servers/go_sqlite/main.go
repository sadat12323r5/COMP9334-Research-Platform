...
func newCsvLogger(path string) (*csvLogger, error) {
	...
	if info.Size() == 0 {
		_ = w.Write([]string{
			...
			"route", "method", "pid", "worker_id", "body_parse_ms", "insert_ms",
			"select_ms", "db_ms", "write_response_ms",
		})
		w.Flush()
	}
...
func (l *csvLogger) log(row traceRow) {
	...
	_ = l.writer.Write([]string{
		...
		fmt.Sprintf("%.6f", row.bodyParseMs),
		fmt.Sprintf("%.6f", row.insertMs),
		fmt.Sprintf("%.6f", row.selectMs),
		fmt.Sprintf("%.6f", row.dbMs),
		fmt.Sprintf("%.6f", row.writeResponseMs),
	})
	l.writer.Flush()
}
...
func doService(db *sql.DB, sensorID int, value float64) (serviceResult, error) {
	...
	selectMs := float64(time.Since(selectStart).Microseconds()) / 1000.0
	return serviceResult{RowsRead: n, Mean: mean, InsertID: insertID, InsertMs: insertMs, SelectMs: selectMs}, nil
}
...
func worker(workerID int, queue <-chan job, db *sql.DB, logger *csvLogger) {
	...
	for j := range queue {
		...
		if j.r.ContentLength > 0 {
			...
			bodyParseMs = float64(time.Since(bodyParseStart).Microseconds()) / 1000.0
		}
		...
		writeResponseMs := float64(responseEnd.Sub(writeStart).Microseconds()) / 1000.0
		responseMs := float64(responseEnd.Sub(j.arrival).Microseconds()) / 1000.0

		logger.log(traceRow{
			arrivalNs:       j.arrivalNs,
			serviceMs:       serviceMs,
			queueMs:         float64(workerStart.Sub(j.arrival).Microseconds()) / 1000.0,
			responseMs:      responseMs,
			statusCode:      statusCode,
			route:           j.r.URL.Path,
			method:          j.r.Method,
			workerID:        workerID,
			bodyParseMs:     bodyParseMs,
			insertMs:        result.InsertMs,
			selectMs:        result.SelectMs,
			dbMs:            result.InsertMs + result.SelectMs,
			writeResponseMs: writeResponseMs,
		})
...
func makeHandler(queue chan<- job) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		j := job{w: w, r: r, arrivalNs: arrivalNs, arrival: arrival, responseCh: responseCh}
