...
def log_csv(arrival_ns: int, service_ms: float, queue_ms: float,
            response_ms: float, status: int, route: str = '',
            method: str = '', body_parse_ms: float = 0.0,
            decrypt_ms: float = 0.0, unpack_ms: float = 0.0,
            fir_ms: float = 0.0, pack_ms: float = 0.0,
            encrypt_ms: float = 0.0) -> None:
        ...
        with open(CSV_FILE, 'a') as f:
            ...
            if not _csv_header_written and f.tell() == 0:
                f.write(
                    ...
                    'route,method,pid,body_parse_ms,decrypt_ms,unpack_ms,'
                    'fir_ms,pack_ms,encrypt_ms\n'
                )
            _csv_header_written = True
            f.write(
                f'{arrival_ns},{service_ms:.6f},{queue_ms:.6f},{response_ms:.6f},{status},'
                f'{route},{method},{os.getpid()},{body_parse_ms:.6f},{decrypt_ms:.6f},'
                f'{unpack_ms:.6f},{fir_ms:.6f},{pack_ms:.6f},{encrypt_ms:.6f}\n'
            )
...
def process():
    ...
    body_parse_ms = (time.perf_counter() - parse_start) * 1000
    ...
    try:
        ...
        log_csv(arrival_ns, svc_ms, max(0, resp_ms - svc_ms), resp_ms, 422,
                request.path, request.method, body_parse_ms)
    ...
    log_csv(arrival_ns, svc_ms, queue_ms, resp_ms, 200, request.path,
            request.method, body_parse_ms, decrypt_ms, unpack_ms, fir_ms,
            pack_ms, encrypt_ms)
