...
public class DspServer {
    ...
    static class ProcessHandler implements HttpHandler {
        ...
        public void handle(HttpExchange exchange) throws IOException {
            ...
            long bodyReadStart = System.nanoTime();
            String body = new String(exchange.getRequestBody().readAllBytes(), StandardCharsets.UTF_8);
            double bodyReadMs = (System.nanoTime() - bodyReadStart) / 1e6;
            long bodyParseStart = System.nanoTime();
            ...
            double bodyParseMs = (System.nanoTime() - bodyParseStart) / 1e6;
            ...
            try {
                ...
                decryptMs = (System.nanoTime() - t0) / 1e6;
            } catch (Exception e) {
                ...
                logCsv(arrivalNs, svcMs, Math.max(0, rspMs - svcMs), rspMs, 422,
                    ...
                    bodyReadMs, bodyParseMs, 0, 0, 0, 0, 0);
            ...
            double unpackMs = (System.nanoTime() - t0) / 1e6;

            // FIR convolution
            t0 = System.nanoTime();
            double[] filtered = firConvolve(signal, KERNEL);
            double firMs = (System.nanoTime() - t0) / 1e6;
            ...
            double packMs = (System.nanoTime() - t0) / 1e6;
            ...
            double encryptMs = (System.nanoTime() - t0) / 1e6;
            ...
            logCsv(arrivalNs, svcMs, queueMs, rspMs, 200,
                exchange.getRequestURI().getPath(), exchange.getRequestMethod(),
                bodyReadMs, bodyParseMs, decryptMs, unpackMs, firMs, packMs, encryptMs);
    ...
    static synchronized void logCsv(long arrivalNs, double svcMs, double queueMs,
        ...
        logCsv(arrivalNs, svcMs, queueMs, rspMs, status, "", "", 0, 0, 0, 0, 0, 0, 0);
    }

    static synchronized void logCsv(long arrivalNs, double svcMs, double queueMs,
                                     double rspMs, int status, String route, String method,
                                     double bodyReadMs, double bodyParseMs, double decryptMs,
                                     double unpackMs, double firMs, double packMs,
                                     double encryptMs) {
        ...
        try {
            ...
            try (FileWriter fw = new FileWriter(f, true)) {
                if (writeHeader) {
                    fw.write("arrival_unix_ns,service_ms,queue_ms,response_ms,status_code,route,method,pid,worker_id,body_read_ms,body_parse_ms,decrypt_ms,unpack_ms,fir_ms,pack_ms,encrypt_ms\n");
                }
                fw.write(String.format("%d,%.6f,%.6f,%.6f,%d,%s,%s,%d,%s,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f%n",
                    arrivalNs, svcMs, queueMs, rspMs, status, route, method,
