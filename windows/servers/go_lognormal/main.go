...
func main() {
	...
	mux.HandleFunc("/", func(w http.ResponseWriter, r *http.Request) {
		...
		writeResponseMs := responseEnd.Sub(writeStart).Seconds() * 1000
		...
		record := []string{
			...
			strconv.Itoa(os.Getpid()),
			"0",
			strconv.FormatFloat(res.targetDemand.Seconds()*1000, 'f', 6, 64),
			strconv.FormatFloat(serviceMs, 'f', 6, 64),
			strconv.FormatFloat(writeResponseMs, 'f', 6, 64),
		}
