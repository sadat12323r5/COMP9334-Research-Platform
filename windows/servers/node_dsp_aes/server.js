...
function startWorker() {
    ...
    if (CSV_FILE) {
        ...
        if (needsHeader) {
            csvStream.write(
                ...
                'route,method,pid,body_read_ms,body_parse_ms,decrypt_ms,unpack_ms,' +
                'fir_ms,pack_ms,encrypt_ms\n'
            );
        }
    }

    function logCsv(arrivalNs, serviceMs, queueMs, responseMs, status, stages = {}) {
        if (!csvStream) return;
        const fields = [
            stages.route || '',
            stages.method || '',
        ...
        csvStream.write(
            ...
            fields.map(v => typeof v === 'number' ? v.toFixed(6) : String(v)).join(',') +
            '\n'
        );
    }
    ...
    const server = http.createServer((req, res) => {
        ...
        const chunks = [];
        const bodyReadStart = process.hrtime.bigint();
        req.on('data', c => chunks.push(c));
        req.on('end', () => {
            const bodyReadMs = Number(process.hrtime.bigint() - bodyReadStart) / 1e6;
            const bodyParseStart = process.hrtime.bigint();
            let parsed;
            try { parsed = JSON.parse(Buffer.concat(chunks)); }
            catch (_) { return sendJson(res, 400, { error: 'invalid_json' }); }
            const bodyParseMs = Number(process.hrtime.bigint() - bodyParseStart) / 1e6;
            ...
            try {
                ...
                var decryptMs = Number(process.hrtime.bigint() - t0) / 1e6;
            } catch (_) {
                ...
                logCsv(arrivalNs, svcMs, Math.max(0, respMs - svcMs), respMs, 422, {
                    route: req.url, method: req.method, bodyReadMs, bodyParseMs
                });
                return;
            }
            ...
            const unpackMs = Number(process.hrtime.bigint() - t0) / 1e6;

            t0 = process.hrtime.bigint();
            const filtered = firConvolve(signal, KERNEL);
            const firMs = Number(process.hrtime.bigint() - t0) / 1e6;

            t0 = process.hrtime.bigint();
            const packed = Buffer.allocUnsafe(nSamples * 4);
            for (let i = 0; i < nSamples; i++) packed.writeFloatLE(filtered[i], i * 4);
            const packMs = Number(process.hrtime.bigint() - t0) / 1e6;

            t0 = process.hrtime.bigint();
            const enc = aesEncrypt(packed);
            const encryptMs = Number(process.hrtime.bigint() - t0) / 1e6;
