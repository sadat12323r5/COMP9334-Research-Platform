...
$traceStageMs = [
    'synthetic_ms' => 0.0,
    'body_parse_ms' => 0.0,
    'next_id_lock_wait_ms' => 0.0,
    'next_id_io_ms' => 0.0,
    'append_lock_wait_ms' => 0.0,
    'append_io_ms' => 0.0,
    'read_lock_wait_ms' => 0.0,
    'read_io_ms' => 0.0,
];
...
function elapsed_ms(int $startNs, int $endNs): float {
    return max(0.0, ($endNs - $startNs) / 1_000_000.0);
}
...
function next_message_id_fast(string $path): int {
    ...
    $lockStartNs = hrtime(true);
    if (!flock($fp, LOCK_SH)) { fclose($fp); return 1; }
    $GLOBALS['traceStageMs']['next_id_lock_wait_ms'] += elapsed_ms($lockStartNs, hrtime(true));

    $ioStartNs = hrtime(true);
    $size    = filesize($path);
    $seekPos = max(0, $size - 4096);
    fseek($fp, $seekPos);
    $chunk = stream_get_contents($fp);
    $GLOBALS['traceStageMs']['next_id_io_ms'] += elapsed_ms($ioStartNs, hrtime(true));
    flock($fp, LOCK_UN);
    fclose($fp);
...
function append_message_bounded(string $path, array $msg, int $maxMessages): void {
    ...
    $GLOBALS['traceStageMs']['append_lock_wait_ms'] += elapsed_ms($lockStartNs, hrtime(true));
    ...
    fflush($fp);
    $GLOBALS['traceStageMs']['append_io_ms'] += elapsed_ms($ioStartNs, hrtime(true));
    flock($fp, LOCK_UN);
    fclose($fp);
}
...
function read_messages(string $path, string $room, int $sinceId, int $limit): array {
    ...
    $GLOBALS['traceStageMs']['read_lock_wait_ms'] += elapsed_ms($lockStartNs, hrtime(true));
    ...
    $GLOBALS['traceStageMs']['read_io_ms'] += elapsed_ms($ioStartNs, hrtime(true));
    flock($fp, LOCK_UN);
    fclose($fp);
...
function log_trace(string $path, int $arrivalNs, float $serviceMs, float $queueMs,
                   float $responseMs, int $statusCode, array $stageMs,
                   string $route, string $method): void {
    $dir = dirname($path);
    if (!is_dir($dir)) mkdir($dir, 0777, true);
    ...
    if ($stat !== false && $stat['size'] === 0) {
        fwrite($fp, "arrival_unix_ns,service_ms,queue_ms,response_ms,status_code,route,method,pid,synthetic_ms,body_parse_ms,next_id_lock_wait_ms,next_id_io_ms,append_lock_wait_ms,append_io_ms,read_lock_wait_ms,read_io_ms\n");
    }
    fwrite($fp, sprintf("%d,%.6f,%.6f,%.6f,%d,%s,%s,%d,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f\n",
        $arrivalNs, $serviceMs, $queueMs, $responseMs, $statusCode,
        str_replace(',', ';', $route), str_replace(',', ';', $method), getmypid(),
        (float)($stageMs['synthetic_ms'] ?? 0.0),
        (float)($stageMs['body_parse_ms'] ?? 0.0),
        (float)($stageMs['next_id_lock_wait_ms'] ?? 0.0),
        (float)($stageMs['next_id_io_ms'] ?? 0.0),
        (float)($stageMs['append_lock_wait_ms'] ?? 0.0),
        (float)($stageMs['append_io_ms'] ?? 0.0),
        (float)($stageMs['read_lock_wait_ms'] ?? 0.0),
        (float)($stageMs['read_io_ms'] ?? 0.0)));
    fflush($fp);
    flock($fp, LOCK_UN);
    fclose($fp);
}
...
$traceStageMs['synthetic_ms'] = $actualServiceMs;
...
if ($csvFile !== '') {
    register_shutdown_function(
        function() use ($requestStartNs, $arrivalNs, $csvFile, &$actualServiceMs, &$traceStageMs, &$path, &$method) {
            ...
            log_trace($csvFile, $arrivalNs, $actualServiceMs, $queueMs,
                      $totalResponseMs, http_response_code(), $traceStageMs, $path ?? '', $method ?? '');
        }
    );
}
...
if ($path === '/send' && $method === 'POST') {
    ...
    $traceStageMs['body_parse_ms'] += elapsed_ms($bodyStartNs, hrtime(true));
    if (!is_array($body)) {
